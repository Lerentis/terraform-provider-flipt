...
1. **No Authentication** - For testing or unauthenticated Flipt instances, simply omit the `token` and `jwt` attributes.

2. **Static Token Authentication** - Use the `token` attribute to provide a static authentication token. The provider will send this as a Bearer token in the `Authorization` header:
   ```
   Authorization: Bearer <token>
   ```

3. **JWT Authentication** - Use the `jwt` attribute to provide a JWT token. The provider will send this in the `Authorization` header:
   ```
   Authorization: JWT <token>
   ```

**Note**: You can only specify one authentication method at a time. Providing both `token` and `jwt` will result in an error.
